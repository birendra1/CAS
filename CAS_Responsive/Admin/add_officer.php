    ...
    $type = substr($file, strrpos($file, '.')+1);
		...
		if($tmpname != "") {
			if (move_uploaded_file($tmpname, $path)) {
					  ...
					  $mailto = $email;
                       ...
                       if(!$mail->Send())
                       ...
                       else
                         {
				           ...
				           echo "<script> alert(\"Officer is Successfully added \") </script>";
			 ...
			 $addQuery = "INSERT INTO officer(id,name,address,phone,email,state,district) VALUES('$id','$name','$address','$phone','$email','$state','$district')" ;
				if (mysqli_query($conn,$addQuery)) {
					  $mailto = $email;
   			         $mailSub = "Registratered to CAS";
   			 		 $mailMsg = "Hi you are registere to CAS having officerid : ".$id." and password : ".$password." Please log in and change the password.";
